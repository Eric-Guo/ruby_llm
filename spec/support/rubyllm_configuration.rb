...
RSpec.shared_context 'with configured RubyLLM' do
  before do
    RubyLLM.configure do |config|
      ...
      config.dify_api_base = ENV.fetch('DIFY_API_BASE', 'https://dify.example.com')
      config.dify_api_key = ENV.fetch('DIFY_API_KEY', 'test')
      config.dify_user = ENV.fetch('DIFY_USER', 'test-user')
